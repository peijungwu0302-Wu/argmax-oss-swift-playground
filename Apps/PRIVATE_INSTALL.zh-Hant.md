...
請沿用原來的 SideStore 與 Apple 帳號，不要刪除「課堂逐字稿」或更換 Bundle ID。v1.8.2 延續 App 內版本檢查及 SideStore 跳轉，功能與發布狀態見 [v1.8.2 說明](UPDATE-1.8.2.zh-Hant.md)。
...
5. 從 [GitHub Release v1.8.2](https://github.com/peijungwu0302-Wu/argmax-oss-swift-playground/releases/tag/v1.8.2) 下載 `LectureTranscriber-v1.8.2.ipa`；不需登入 GitHub，也不需解開 Actions artifact ZIP。
...
v1.8.2 延續 iPhone／iPad 共用 IPA。主 App Bundle ID 保持 `com.peijungwu0302.lecturetranscriber`，並只保留既有 Live Activity Widget Extension；使用相同 Apple 帳號與簽署工具覆蓋更新，避免另建識別碼或刪除資料。

本版以 PiP 作為跨 App 即時字幕，加入完整逐字稿入口、PiP 即時排版調整與 Live Activity 修正；既有模型、錄音、逐字稿版本、翻譯及手動修改都保留。詳細內容見 [v1.8.2 說明](UPDATE-1.8.2.zh-Hant.md)。
