...
enum SmartNotes {
    ...
    static func generate(_ lecture: LectureSession, progress: @escaping @Sendable (String) -> Void) async throws -> Result {
        if #available(iOS 26.0, *), case .available = SystemLanguageModel.default.availability {
            return try await summarize(lecture, progress: progress)
        }
        return Result(text: MeetingNotes.outline(lecture), kind: "Apple Intelligence 未就緒，已產生原文整理")
    }
    @available(iOS 26.0, *)
    private static func summarize(_ lecture: LectureSession, progress: @escaping @Sendable (String) -> Void) async throws -> Result {
        ...
        for (index, text) in chunks.enumerated() {
            ...
            let response = try await model.respond(to: "整理這段逐字稿：\n\n" + text)
